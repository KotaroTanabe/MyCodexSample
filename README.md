...
ファイルが保存されます。
同様に「MJAIログダウンロード」ボタンから、Mortal互換の `.jsonl` 形式で牌譜を取得できます。
